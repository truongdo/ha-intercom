...
"${TAR_CREATE[@]}" -C deploy -czf - config.example.toml live-intercom.service remote-setup.sh ringtone.wav journald-live-intercom.conf | ssh "$HOST" "tar -xzf - -C $DEST/deploy"
